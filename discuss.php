    ...
    <main class="main-content">
        ...
        <div class="db-viewer-container">
            ...
            <div class="db-section">
                ...
                <?php
                $sql = "SELECT * FROM discuss ORDER BY date DESC, time DESC";
                $result = $mysqli->query($sql);
                
                if ($result) {
                    $count = $result->num_rows;
                    echo '<div class="db-count">Total Discussions: ' . $count . '</div>';
                    ...
                    if ($count > 0) {
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo '<td class="db-content-cell">' . htmlspecialchars(substr($row['content'], 0, 100)) . (strlen($row['content']) > 100 ? '...' : '') . '</td>';
                    ...
                    echo '<div class="db-error">Error querying discussions: ' . htmlspecialchars($mysqli->error) . '</div>';
                }
                ?>
            </div>
            ...
            <div class="db-section">
                <h2>💭 Replies Table</h2>
                <?php
                $sql = "SELECT * FROM user_discuss ORDER BY id ASC";
                $result = $mysqli->query($sql);
                
                if ($result) {
                    $count = $result->num_rows;
                    echo '<div class="db-count">Total Replies: ' . $count . '</div>';
                    ...
                    echo '<div class="db-error">Error querying replies: ' . htmlspecialchars($mysqli->error) . '</div>';
                }
                ?>
            </div>
            ...
            <div class="db-section">
                ...
                // Total discussions
                $result = $mysqli->query("SELECT COUNT(*) as cnt FROM discuss");
                if ($result) {
                    $row = $result->fetch_assoc();
                    $stats['total_discussions'] = $row['cnt'];
                }
                
                // Total replies
                $result = $mysqli->query("SELECT COUNT(*) as cnt FROM user_discuss");
                if ($result) {
                    $row = $result->fetch_assoc();
                    $stats['total_replies'] = $row['cnt'];
                }
                
                // Discussions by category
                $result = $mysqli->query("SELECT category, COUNT(*) as cnt FROM discuss GROUP BY category");
                if ($result) {
                    $stats['by_category'] = [];
                    while ($row = $result->fetch_assoc()) {
                ...
                // Total views
                $result = $mysqli->query("SELECT SUM(views) as total FROM discuss");
                if ($result) {
                    $row = $result->fetch_assoc();
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (container) {
                ...
                // 測試 API 連接
                fetch('discussions_api.php?action=list')
                    .then(res => {
